...
#define STRING_TEMPLATE(filename)                                                                  \
	std::string_view(&PREFIX(filename##_start), &PREFIX(filename##_end) - &PREFIX(filename##_start))

#define FILE_TEMPLATE(filename)                                                                    \
	TemplateFile {                                                                                   \
		"ArgGrammar" #filename, STRING_TEMPLATE(filename)                                              \
	}

const TemplateFile templateFiles[] = { FILE_TEMPLATE(Driver_cpp),  FILE_TEMPLATE(Driver_hpp),
